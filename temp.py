...
def train_step(train_parameters):
    ...
    for hr, lr, cond, name, _ in train_bar:
        i += 1
        # lr = lr.to(DEVICE)
        # hr = hr.to(DEVICE)
        # cond = cond.to(DEVICE)
        
        if hr.shape[-1] != 43200 or lr.shape[-1] != 43200:
            print(f"HR{hr.shape[-1]} LR{lr.shape[-1]}")
            print(name)
        # with autocast():
...
def main():
    ...
    TPARAMS['train_dataloader'] = DataLoader(train_dataset, batch_size = 1, 
